...
if (speed = 0) 
{
	image_index = idle_index;	// once we have an idle animation, use that here instead
	image_speed = 0;
}
...
else
{
	...
	if (dist_to_player >= enemy_detection_range && enemy_has_detected_player)
	{
		...
		if (enemy_forget_timer <= 0)
		{
			...
			enemy_detection_time = 30; // the time that the '!' is displayed for
	...
	if (dist_to_player <= enemy_detection_range || enemy_has_detected_player)
	{
		...
		if (dist_to_player >= enemy_melee_range - buffer && dist_to_player <= enemy_melee_range + buffer)
			speed = 0;
		else if (dist_to_player > enemy_ranged_range / 2)
		{
		...
		{
			if (dist_to_player < enemy_ranged_range / 3)
			{
	...
	if (enemy_can_ranged_attack)
	{
		RangedAttack();
		enemy_can_ranged_attack = false;
		enemy_ranged_attack_timer = enemy_ranged_attack_reset_time;
		inactive = true;
		alarm_set(0, irandom_range(8, 22)); // alarm simply sets the enemy to active again after 15 frames have passed
	}
	else
	{
		if (enemy_ranged_attack_timer > 0)
		{
			enemy_ranged_attack_timer -= 1;
		}
		else
			if (enemy_within_ranged_range)
				enemy_can_ranged_attack = true;
	}
}
